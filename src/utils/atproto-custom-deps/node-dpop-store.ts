...
export function toDpopKeyStore<K extends string, V extends { dpopKey: Key }>(
  ...
  return {
    async set(sub: K, { dpopKey, ...data }: V, options?: Record<string, unknown>) {
      const dpopJwk = dpopKey.privateJwk
      if (!dpopJwk) throw new Error('Private DPoP JWK is missing.')

      await store.set(sub, { ...data, dpopJwk }, options)
    },

    async get(sub: K, options) {
      const result = await store.get(sub, options)
