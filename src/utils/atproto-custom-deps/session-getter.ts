...
import { TokenInvalidError, TokenRevokedError, OAuthResponseError, TokenRefreshError, TokenSet, OAuthServerFactory } from '@atproto/oauth-client-node'
...
export class SessionGetter extends CachedGetter<AtprotoDid, Session> {
  ...
  async setStored(sub: string, session: Session, options?: Record<string, unknown>) {
    // Prevent tampering with the stored value
    if (sub !== session.tokenSet.sub) {
      throw new TypeError('Token set does not match the expected sub')
    }
    await super.setStored(sub, session, options)
